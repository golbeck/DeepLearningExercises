...
def sgd_optimization_mnist(learning_rate=0.13, n_epochs=1000,dataset='mnist.pkl.gz',batch_size=600):
    ...
    print >> sys.stderr, ('The code for file logistic_sgd_ipy.py ran for %.1fs' % ((end_time - start_time)))
...
#load data and parse into training, test and validation sets
datasets = load_data(dataset)
...
n_valid_batches = valid_set_x.get_value(borrow=True).shape[0] / batch_size
n_test_batches = test_set_x.get_value(borrow=True).shape[0] / batch_size
#save the train set inputs into a numpy array
temp_x=numpy.asanyarray(train_set_x.get_value())
temp_y=numpy.asanyarray(train_set_y.eval())
#example of one observation
temp_x[0,:]
train_set_x.get_value()[0,:]
train_set_y.eval()[0:10]
